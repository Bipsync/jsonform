...
var numberFieldTemplate = function (type) {
  return {
    'template': '<input type="' + type + '" ' +
      '<%= (fieldHtmlClass ? "class=\'" + fieldHtmlClass + "\' " : "") %>' +
      'name="<%= node.name %>" value="<%= escape(value) %>" id="<%= id %>"' +
...
jsonform.elementTypes = {
  ...
  'number': numberFieldTemplate('number'),
  ...
  'radiobuttons': {
    'template': '<div id="<%= node.id %>" '+' class="<%= elt.htmlClass ? " " + elt.htmlClass : "" %>">' +
      '<% _.each(node.options, function(key, val) { %>' +
        '<label class="radio btn<%= elt.fieldHtmlClass ? " " + elt.fieldHtmlClass : "" %>">' +
  ...
  'checkboxbuttons': {
    'template': '<div id="<%= node.id %>" class="<%= elt.htmlClass ? " " + elt.htmlClass : "" %>"><%= choiceshtml %></div>',
    'fieldtempate': true,
    'inputfield': true,
    'onBeforeRender': function (data, node) {
      // Build up choices from the enumeration list
      var choices = null;
      var choiceshtml = null;
      var template = '<label class="checkbox btn' + (data.elt.fieldHtmlClass ? " " + data.elt.fieldHtmlClass : "") + '">' +
      ...
      _.each(choices, function (choice, idx) {
        choiceshtml += _.template(template, {
          name: node.key + '[' + idx + ']',
          checked: _.include(node.value, choice),
          value: choice,
          title: node.formElement.titleMap ? node.formElement.titleMap[choice] : choice,
          node: node
  ...
  'help':{
    'template':'<span class="help-block" style="padding-top:5px"><%= node.helpvalue || elt.helpvalue %></span>',
    'fieldtemplate': true
  },
  'msg': {
    'template': '<%= elt.msg %>'
  ...
  for (var i = 0; i < formArray.length; i++) {
    ...
    if (eltSchema._jsonform_checkboxes_as_array) {
      ...
      if (arrayMatch) {
        ...
        if (eltSchema._jsonform_checkboxes_as_array === 'value' && formArray[i].value !== false) {
...
formTree.prototype.buildFromLayout = function (formElement, context) {
  ...
  if (formElement.key) {
    ...
    if ((formElement.type === 'checkboxes' || formElement.type === 'checkboxbuttons') && schemaElement.items) {
      ...
      if (itemsEnum) {
        schemaElement.items._jsonform_checkboxes_as_array = formElement.type === 'checkboxes' ? true : 'value';
      }
      if (!itemsEnum && schemaElement.items[0]) {
        itemsEnum = schemaElement.items[0]['enum'];
        if (itemsEnum) {
          schemaElement.items[0]._jsonform_checkboxes_as_array = formElement.type === 'checkboxes' ? true : 'value';
        }
      }
    }
